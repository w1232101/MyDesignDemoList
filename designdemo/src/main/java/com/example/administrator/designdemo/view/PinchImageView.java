...
public class PinchImageView extends ImageView {
	...
	private GestureDetector mGestureDetector = new GestureDetector(PinchImageView.this.getContext(),
			new GestureDetector.SimpleOnGestureListener() {
				...
				public boolean onDoubleTap(MotionEvent e) {
					return doubleTap(e.getX(), e.getY());
					// return true;//wujiaxing
				}
		...
		if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
			...
			if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
				...
				if (mPinchMode == PINCH_MODE_SCROLL) {
					...
					// 保存缩放点中点
					float[] lineCenter = MathUtils.getCenterPoint(event.getX(0), event.getY(0), event.getX(1),
							event.getY(1));
	...
	private boolean doubleTap(float x, float y) {
		// 不允许动画过程中再触发
